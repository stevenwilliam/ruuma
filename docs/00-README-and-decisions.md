...
| D31 | 2026-08-03 | **Dish photography is placeholder art, generated by `tools/genassets`** — one deterministic card per SKU, served from `web/public/dish/<SKU>.jpg`. Cards are **textless** so the ID/EN catalogues stay the source of every string. Also generated: square app icons (the 1.94:1 wordmark was being squashed into the square favicon slot, which read as a stretched logo) and `ruuma-logo-white.png` for the emerald header and footer, where the emerald wordmark is invisible. **Real photography replaces these before launch**; the admin upload path is unchanged and still writes to MinIO. | No licensed food photography exists yet, and unlicensed stock on a site taking real orders is a liability. | 10, 11 |
