...
| D34 | 2026-08-12 | **Every mutating control is an `AsyncButton`** — disabled for the life of its request, `aria-busy` while running, and confirming before anything that cannot be walked back (payment verification, blackout, staff/store deactivation, 86, hand-over). Found by auditing the SPA against the `ui-ux-pro-max` rule set (D33). This is a correctness fix, not polish: each mutating call site passes a fresh `crypto.randomUUID()` idempotency key, so a double click was **two distinct operations** rather than one retried, and verifying a payment twice writes two immutable `payment_events` rows (D26). Pinned by `web/src/__tests__/async-button.test.tsx`. Same pass: decorative chilli emoji get `aria-hidden` so a screen reader stops reading "hot pepper" per level. Also cleared a quality gate that had been red since D31 — 5 gosec findings in `tools/*` (3 real permission tightenings, 2 false positives annotated `#nosec` with reasons) and the `nanoid` advisory GHSA-2v37-7h3g-55p8 (3.3.16 → 3.3.18, lockfile only). **Recorded gap:** admin copy is still inline English rather than message catalogues, contradicting CLAUDE.md §10 — logged in `10` §5.1, not fixed. | Steven's directive to rescan, rework, retest and redeploy against the new skill. | 00, 10 |
